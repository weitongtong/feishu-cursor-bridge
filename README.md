...
```text
发送文本 → Cursor 以 plan 模式分析代码并生成方案
继续发文本 → Cursor 在同一会话中细化方案
/run → Cursor 以 agent 模式执行方案（允许改文件）
```

1. 直接发送任务描述，Cursor 会自动以 plan 模式规划方案
2. 查看方案后，继续发消息可以细化和补充需求
3. 方案满意后，发送 `/run` 让 Cursor 执行
...
| 输入            | 说明                                                       |
| --------------- | ---------------------------------------------------------- |
| 直接发送文本    | 自动以 plan 模式分析代码并生成方案                         |
| `/run [指示]`   | 开始执行并允许改文件；不带参数则执行之前讨论的方案         |
| `/ask [问题]`   | 只读问答，不改任何文件                                     |
